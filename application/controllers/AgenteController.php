<?php 
class AgenteController extends Escola_Controller_Logado {

	public function indexAction() {
        $session = Escola_Session::getInstance();
        if (isset($session->id_agente)) {
            unset($session->id_agente);
        }
        $dados = $session->atualizaFiltros(array("filtro_codigo", "filtro_matricula", "filtro_cpf", "filtro_nome"));
		$tb = new TbAgente();
		$dados["pagina_atual"] = $this->_getParam("page");
		$this->view->registros = $tb->listar_por_pagina($dados);
        $this->view->dados = $dados;
		$button = Escola_Button::getInstance();
		$button->setTitulo("AGENTES DE TRÂNSITO");
	...
	public function editarAction() {
		...
		if ($this->view->registro->getId()) {
			$button->setTitulo("CADASTRO DE AGENTES DE TRÂNSITO - ALTERAR");
		} else {
			$button->setTitulo("CADASTRO DE AGENTES DE TRÂNSITO - INSERIR");
		}
		...
		if ($id) {
			...
			if ($registro) {
                ...
                $button->addFromArray(array("titulo" => "Excluir",
                                            ...
                                            "class" => "link_excluir",
                                            "params" => array("id" => $id)));
                $button->addFromArray(array("titulo" => "Autos de Infração",
                                            "controller" => $this->_request->getControllerName(),
                                            "action" => "autoinfracao",
        ...
        if ($id_agente) {
            ...
            if ($registro) {
                ...
                $button->addFromArray(array("titulo" => "Pesquisar",
                                            ...
                                            "img" => "icon-search",
                                            "params" => array("id" => 0)));
                $button->addFromArray(array("titulo" => "Devolver",
                                            "onclick" => "devolver()",
                    ...
                    if (isset($dados["flag"]) && ($dados["flag"] == "salvar")) {
                        if (isset($dados["result"]) && is_array($dados["result"]) && count($dados["result"])) {
                            ...
                            foreach ($dados["result"] as $id_auto_infracao =>  $result) {
                                ...
                                if ($ai && $ai->entregue()) {
                                    $flag = $ai->devolver(array("id_auto_infracao_devolucao_status" => $result["aids"], 
