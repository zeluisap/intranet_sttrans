<?php 
class BaixamotivoController extends Escola_Controller_Logado {

	public function indexAction() {
		$tb = new TbBaixaMotivo();
		$page = $this->_getParam("page");
		$this->view->registros = $tb->listar_por_pagina(array("pagina_atual" => $page));
		$button = Escola_Button::getInstance();
		$button->setTitulo("MOTIVOS DA BAIXA");
	...
	public function editarAction() {
		...
		if ($this->view->registro->getId()) {
			$button->setTitulo("CADASTRO DE MOTIVO DE BAIXA - ALTERAR");
		} else {
			$button->setTitulo("CADASTRO DE MOTIVO DE BAIXA - INSERIR");
		}
	...
	public function viewAction() {
		...
		if ($id) {
			...
			if ($registro) {
				$this->view->registro = $registro;
				$button = Escola_Button::getInstance();
				$button->setTitulo("VISUALIZAR MOTIVO DA BAIXA");
