<?php
class PrioridadeController extends Escola_Controller_Logado {
	...
	public function indexAction() {
		...
		$button->setTitulo("PRIORIDADES DE DOCUMENTOS");
	...
	public function editarAction() {
		...
		if ($this->view->registro->getId()) {
			$button->setTitulo("CADASTRO DE PRIORIDADES - ALTERAR");
		} else {
			$button->setTitulo("CADASTRO DE PRIORIDADES - INSERIR");
		}
	...
	public function excluirAction() {
		...
		if ($id) {
			...
			if ($registro) {
				if ($registro->normal()) {
					$this->_flashMessage("FALHA AO EXECUTAR OPERAÇÃO, PRIORIDADE NORMAL NÃO PODE SER EXCLUÍDA!");
				} else {
					$registro->delete();
	...
	public function viewAction() {
		...
		if ($id) {
			...
			if ($registro) {
				$this->view->registro = $registro;
				$button = Escola_Button::getInstance();
				$button->setTitulo("VISUALIZAR PRIORIDADE");
