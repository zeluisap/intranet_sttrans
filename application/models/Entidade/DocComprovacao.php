<?php
class DocComprovacao extends Escola_Entidade {
    ...
    public function getErrors() {
        ...
        $rg = $this->getTable()->fetchAll(" chave = '{$this->chave}' and id_doc_comprovacao <> '" . $this->getId() . "' ");
        if ($rg && count($rg)) {
            $msgs[] = "TIPO DE DOCUMENTO DE COMPROVAÇÃO JÁ CADASTRADO!";
        }
		if (count($msgs)) {
			return $msgs;
		}
    ...
    public function getDeleteErrors() {
        $msgs = array();
        $registros = $this->findDependentRowset("TbVinculoLoteOcorrenciaPgto");
