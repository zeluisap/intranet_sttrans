<?php
class TarifaTipo extends Escola_Entidade {
    ...
    public function getErrors() {
        ...
        $rg = $this->getTable()->fetchAll(" chave = '{$this->chave}' and id_tarifa_tipo <> '" . $this->getId() . "' ");
        if ($rg && count($rg)) {
            $msgs[] = "TIPO DE TARIFA JÁ CADASTRADO!";
        }
        if (count($msgs)) {
            return $msgs;
        }
    ...
    public function getDeleteErrors() {
        $msgs = array();
        $registros = $this->findDependentRowset("TbOnibusBdoTarifa");
