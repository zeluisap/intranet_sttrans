<?php
class MaterialTipo extends Escola_Entidade {
    ...
    public function getErrors() {
        ...
        $rg = $this->getTable()->fetchAll(" chave = '{$this->chave}' and id_material_tipo <> '" . $this->getId() . "' ");
        if ($rg && count($rg)) {
            $msgs[] = "TIPO DE MATERIAL DE CONSUMO JÁ CADASTRADO!";
        }
		if (count($msgs)) {
			return $msgs;
		}
    ...
    public function getDeleteErrors() {
        $msgs = array();
        $registros = $this->findDependentRowset("TbMaterialTipoItem");
