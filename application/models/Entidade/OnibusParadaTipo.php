<?php
class OnibusParadaTipo extends Escola_Entidade {
    ...
    public function getErrors() {
        ...
        $rg = $this->getTable()->fetchAll(" chave = '{$this->chave}' and id_onibus_parada_tipo <> '" . $this->getId() . "' ");
        if ($rg && count($rg)) {
            $msgs[] = "TIPO DE PARADA DE ÔNIBUS JÁ CADASTRADA!";
        }
		if (count($msgs)) {
			return $msgs;
		}
    ...
    public function getDeleteErrors() {
        $msgs = array();
        $registros = $this->findDependentRowset("TbOnibusParada");
