<?php
class SetorNivel extends Escola_Entidade {
    
    public function toString() {
        return $this->descricao;
    }
    
    public function eInstituicao() {
        return ($this->chave == "T");
    }
    ...
    public function getErrors() {
        ...
        $rg = $this->getTable()->fetchAll(" chave = '{$this->chave}' and id_setor_nivel <> '" . $this->getId() . "' ");
        if ($rg && count($rg)) {
            $msgs[] = "NÍVEL DE SETOR JÁ CADASTRADO!";
        }
		if (count($msgs)) {
			return $msgs;
		}
    ...
    public function getDeleteErrors() {
        $msgs = array();
        $registros = $this->findDependentRowset("TbSetor");
