<?php
class DespesaTipo extends Escola_Entidade {
    ...
    public function getErrors() {
        ...
        $rg = $this->getTable()->fetchAll(" chave = '{$this->chave}' and id_despesa_tipo <> '" . $this->getId() . "' ");
        if ($rg && count($rg)) {
            $msgs[] = "TIPO DE DESPESA JÁ CADASTRADO!";
        }
		if (count($msgs)) {
			return $msgs;
		}
    ...
    public function normal() {
        return ($this->chave == "NO");
    }
    
    public function despesa_bancaria() {
        return ($this->chave == "DB");
    }
}