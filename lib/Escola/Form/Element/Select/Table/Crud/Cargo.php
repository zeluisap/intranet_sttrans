<?php
class Escola_Form_Element_Select_Table_Crud_Cargo extends Escola_Form_Element_Select_Table_Crud {
...
<script type="text/javascript">
	$(document).ready(function() {
		$("#janela_crud_<?php echo $this->getName(); ?> #bt_crud_submit").click(function() {
            ajax = $.ajax({
                "url" : "<?php echo $view->baseUrl(); ?>/cargo/salvar/format/json/",
                "type" : "POST",
                "data" : { "id_cargo_tipo" : $("#id_cargo_tipo").val(), "descricao" : $("#janela_crud_<?php echo $this->getName(); ?> #descricao").val() },
	...
	function reloadCrud_<?php echo $this->getName(); ?>(default_id) {
        ajax = $.ajax({
            "url" : "<?php echo $view->baseUrl(); ?>/cargo/listartodos/format/json/",
            "type" : "POST",
            "success" : function(obj_view) {
                if (obj_view.result) {
...
<div id="janela_crud_<?php echo $this->getName(); ?>" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-header">
        ...
        <h3 id="myModalLabel">Adicionando Registro - Cargo</h3>
    </div>
...
<?php 
	$ctrl = new Escola_Form_Element_Select_Table("id_cargo_tipo");
	$ctrl->setPkName("id_cargo_tipo");
	$ctrl->setModel("TbCargoTipo");
