<?php
class Escola_Form_Element_Select_Table_Crud_Inline_Municipio extends Escola_Form_Element_Select_Table {
	...
	$(document).ready(function() {
        $("#link_salvar_<?php echo $this->getName(); ?>").click(function() {
            ajax = $.ajax({
                ...
                "type" : "POST",
                "data" : { "descricao" : $("#input_add_<?php echo $this->getName(); ?>").val(), "id_uf": $("#<?php echo $this->_id_uf; ?>").val() },
                "success" : function(obj_view) {
                    if (obj_view.result) {
                        ...
                        if (obj_view.result.id) {
                            reload_crud_<?php echo $this->getName(); ?>(obj_view.result.id, $("#<?php echo $this->_id_uf; ?>").val());
                        }
                    }
                }
            });
        ...
        $("#input_add_<?php echo $this->getName(); ?>").keypress(function(event) {
            ...
            if (event.which == 13) {
                ...
                event.preventDefault();
                $("#link_cancelar_<?php echo $this->getName(); ?>").click();
            }
        });
	});
    
    function reload_crud_<?php echo $this->getName(); ?>(default_id, id_uf) {
        $(".<?php echo $this->getName(); ?>_mensagem_erro").html("");
        $("#linha_<?php echo $this->getName(); ?>").removeClass("warning");    
