<?php
class Escola_Form_Element_Select_Table_Motorista extends Escola_Form_Element_Select_Table {
    
    public function init() {
        parent::init();
        $this->setPkName("id_motorista");
        $this->setModel("TbMotorista");
    }
	
	public function render(Zend_View_Interface $view = null) {
        $tb = new TbCnhCategoria();
        $tps = $tb->listar();
        $motorista = TbMotorista::pegaPorId($this->getValue());
        $txt = "";
        if ($motorista) {
            $pessoa_motorista = $motorista->findParentRow("TbPessoaMotorista");
            if ($pessoa_motorista) {
                $txts = array();
                $txts[] = $motorista->matricula;
...
var <?php echo $this->getName(); ?>_motorista = [];
$(document).ready(function() {
    ...
    $("#janela_<?php echo $this->getName(); ?>").on("show", function() {
        ...
        <?php echo $this->getName(); ?>_atualiza_motorista();
    });
...
function <?php echo $this->getName(); ?>_atualiza_motorista() {
    ...
    var ajax_obj = $.ajax({
        "url" : "<?php echo Escola_Util::getBaseUrl(); ?>/motorista/listarporpagina/format/json/",
        "type" : "POST",
        "data" : { "filtro_id_cnh_categoria": $("#<?php echo $this->getName(); ?>_id_cnh_categoria").val(),
                   ...
                   "filtro_matricula": $("#<?php echo $this->getName(); ?>_matricula").val(),
                   "pagina_atual": $("#<?php echo $this->getName(); ?>_pagina").val(),
                   "qtd_por_pagina": 20 },
        "success" : function(result) {
            <?php echo $this->getName(); ?>_motorista = [];
            if (result.items && result.items.length) {
                for (var i = 0; i < result.items.length; i++) {
                    var item = result.items[i];
                    <?php echo $this->getName(); ?>_motorista[i] = item;
                ...
                $(".link_destino").click(
                    function(event) {
                        event.preventDefault();
                        $("#show_<?php echo $this->getName(); ?>").val(<?php echo $this->getName(); ?>_motorista[$(this).attr("id")].matricula + ' - ' + <?php echo $this->getName(); ?>_motorista[$(this).attr("id")].cnh_numero + " - " + <?php echo $this->getName(); ?>_motorista[$(this).attr("id")].nome + " - " + <?php echo $this->getName(); ?>_motorista[$(this).attr("id")].cnh_categoria);
                        $("#<?php echo $this->getName(); ?>").val(<?php echo $this->getName(); ?>_motorista[$(this).attr("id")].id);
        ...
        <div class="well well-small">
            <fieldset>
                <div class="control-group">
                    <label for="<?php echo $this->getName(); ?>_matricula" class="control-label">Matrícula:</label>
                    <div class="controls">
                        <input type="text" name="<?php echo $this->getName(); ?>_matricula" id="<?php echo $this->getName(); ?>_matricula" value="" size="60" class="filtro span2" />
                    </div>
                </div>
                <div class="control-group">
