...
class Escola_Relatorio_Default_Ttr_CaraterGrupo extends Escola_Relatorio_Default
{
    ...
    public function validarEmitir()
    {
        ...
        if (!isset($this->dados["id_transporte_grupo"]) || !$this->dados["id_transporte_grupo"]) {
            $errors[] = "GRUPO DE TRANSPORTE Nï¿½O INFORMADO!";
        }
        if (isset($this->dados["id_transporte_grupo"]) && $this->dados["id_transporte_grupo"]) {
        ...
        if ($stmt && $stmt->rowCount()) {
            ...
            while ($obj = $stmt->fetch(Zend_Db::FETCH_OBJ)) {
                if ($id_concessao_tipo != $obj->id_concessao_tipo) {
                    ...
                    $worksheet->writeString($linha, 0, utf8_decode($obj->concessao_tipo_descricao), $estilo_cabecalho);
                    $worksheet->mergeCells($linha, 0, $linha, 10);
