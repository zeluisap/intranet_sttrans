...
class Escola_Relatorio_Servico_CP_PR_CARGA extends Escola_Relatorio_Servico
{
    ...
    public function imprimir()
    {
        $ss = $this->registro;
        if (!$ss) {
            throw new Exception("Falha ao Gerar Carteira de Carga! Nenhum ServiÃ§o Encontrado!");
        }
        $pessoa = null;
        ...
        if ($ss->veiculo()) {
            ...
            if (!$tv) {
                throw new Exception("Falha ao Gerar RelatÃ³rio!");
            }
            $v = $tv->pegaVeiculo();
            if (!$v) {
                throw new Exception("Falha ao Gerar RelatÃ³rio!");
        ...
        if (!$v) {
            throw new Exception("Falha, VeÃ­culo Indisponivel!");
        }
        ...
        if (!$pf) {
            throw new Exception("Falha, Pessoa FÃ­sica NÃ£o DisponÃ­vel!");
        }
        ...
        $this->setXY(15, 81.5);
        $this->MultiCell(100, 20, "AUTORIZAÃÃO DE TRÃFEGO No.: {$licenca->codigo}/{$licenca->ano_referencia}", 0, 'C');
        ...
        $this->setXY(17, 122);
        $this->MultiCell(100, 20, "O portador encontra-se cadastrado na STTRANS, para o serviÃ§o de {$txt_transporte}.", 0, 'L');
        ...
        $this->setFont($font_name, "B", 6);
        $this->setXY(17, 127);
        $this->MultiCell(100, 20, "SÃ³ Ã© vÃ¡lida mediante a apresentaÃ§Ã£o da Carteira Nacional de HabilitaÃ§Ã£o.", 0, 'L');

        $this->setFont($font_name, "B", 6);
        $this->setXY(17, 129.5);
        $this->MultiCell(100, 20, "Ã obrigatÃ³rio a apresentaÃ§Ã£o desta, quando solicitado pelos Agentes da Autoridade de TrÃ¢nsito.", 0, 'L');
